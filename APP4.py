                ...
                try:
                    with st.spinner("正在生成SHAP解释图..."):
                        ...
                        try:
                            ...
                            feature_font = ImageFont.truetype(font_path, 14)
                        ...
                        # 添加标题
                        draw.text((img.width // 2 - 100, 10), "特征对预测的影响", fill="black", font=title_font)
                        
                        # 添加特征标签
                        img_height = img.height
                        bar_height = (img_height - 80) / len(feature_labels_with_values)
                        
                        # 计算每个特征标签的Y位置
                        for i, label in enumerate(feature_labels_with_values):
                            y_position = 70 + i * bar_height  # 起始位置(顶部) + 每个条形高度 * 索引
                            draw.text((10, y_position), label, fill="black", font=feature_font)
                        
                        # 添加SHAP值标签
                        for i, value in enumerate(importance_df['SHAP值']):
                            y_position = 70 + i * bar_height
                            sign = "+" if value > 0 else ""
                            text = f"{sign}{value:.2f}"
                            # 根据值的正负确定位置
                            x_position = img.width // 2 + int((value / max_val) * (img.width // 3))
                            x_offset = len(text) * 8 if value < 0 else 10
                            draw.text((x_position - x_offset, y_position), text, fill="black", font=value_font)
                        
                        # 添加基准值文本
                        draw.text((10, img_height - 20), f"基准值 f(x) = {base_value_formatted}", fill="black", font=value_font)
                        
                        # 添加红蓝图例
                        legend_y = img_height - 40
                        # 红色方块 - 增加风险
                        draw.rectangle([(img.width - 150, legend_y), (img.width - 130, legend_y + 15)], fill="#ff4d4d")
                        draw.text((img.width - 125, legend_y), "增加风险", fill="black", font=value_font)
                        # 蓝色方块 - 降低风险
                        draw.rectangle([(img.width - 150, legend_y + 20), (img.width - 130, legend_y + 35)], fill="#2196F3")
                        draw.text((img.width - 125, legend_y + 20), "降低风险", fill="black", font=value_font)
                        
                        # 保存和显示最终图像
                        img.save("shap_effect_plot_with_text.png")
                        st.image("shap_effect_plot_with_text.png")
